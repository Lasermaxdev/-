...
app.put('/api/printers/:id', authenticateToken, checkPermission('printers:edit'), async (req, res) => {
  try {
    const { id } = req.params;
    const updates = req.body;
...
// حذف الطابعة
app.delete('/api/printers/:id', authenticateToken, checkPermission('printers:delete'), async (req, res) => {
  try {
    const { id } = req.params;

    // التحقق من وجود الطابعة
    const [existingPrinter] = await pool.query(
      'SELECT id, status FROM printers WHERE id = ?',
      [id]
    );
    ...
    // التحقق من أن الطابعة غير مرتبطة بعمليات بيع أو صيانة نشطة
    const [activeSales] = await pool.query(
      'SELECT id FROM sales WHERE printer_id = ? AND status != "cancelled"',
      [id]
    );

    if (activeSales.length > 0) {
      return res.status(400).json({ error: 'لا يمكن حذف الطابعة لارتباطها بعمليات بيع نشطة' });
    }

    const [activeMaintenance] = await pool.query(
      'SELECT id FROM maintenance_requests WHERE printer_id = ? AND status IN ("pending", "in_progress")',
      [id]
    );

    if (activeMaintenance.length > 0) {
      return res.status(400).json({ error: 'لا يمكن حذف الطابعة لارتباطها بطلبات صيانة نشطة' });
    }

    // حذف السجلات المرتبطة والطابعة
    await pool.query('START TRANSACTION');

    // حذف طلبات الصيانة المكتملة أو الملغاة
    await pool.query(
      'DELETE FROM maintenance_requests WHERE printer_id = ? AND status IN ("completed", "cancelled")',
      [id]
    );

    // حذف عمليات البيع الملغاة
    await pool.query(
      'DELETE FROM sales WHERE printer_id = ? AND status = "cancelled"',
      [id]
    );
